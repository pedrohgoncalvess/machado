...
from machado.utils.env_var import get_env_var


class RelationalLogger:
    def __init__(self, log_format: str | None = None):
        self._log_file_dir_ = (
            get_env_var("LOG_PATH") if get_env_var("LOG_PATH") is not None else "."
        )
        self._log_file_format_ = (
            f".{re.sub(r'[^a-zA-Z0-9]', '', log_format)}.log" if log_format else ".log"
        )
        ...
        self.format = Template(
            f"""$mode {self.separator} {datetime.now().strftime(self.time_format)} {self.separator} $primary_message {self.separator} $obs """
        )

        if not os.path.exists(self._log_path_):
            with open(self._log_path_, "w") as f:
                f.write(f" {self.separator} ".join(self.headers))
    ...
    def error(self, error_type: str, exception: str | None = None):
        self._write_log_(
            self.format.substitute(
                ...
                obs=exception if exception else "null",
            )
        )

    def info(self, info_type: str, message: str):
        self._write_log_(
            self.format.substitute(mode="INFO", primary_message=info_type, obs=message)
        )
